  ...
  io.on('connection', (socket) => {
    ...
    socket.on('start-tracking', async (data) => {
      ...
      if (isRedisAvailable()) {
        ...
        memoryRunners.set(userId.toString(), runnerData);
      }
    ...
    socket.on('get-active-runners', async (data) => {
      const { bounds } = data; // { minLat, minLng, maxLat, maxLng }
      
      let allRunners = [];
      if (isRedisAvailable()) {
        const rawRunners = await redisClient.hGetAll(RUNNER_PRESENCE_KEY);
        allRunners = Object.values(rawRunners).map(r => JSON.parse(r));
      } else {
        allRunners = Array.from(memoryRunners.values());
      }

      const runnersInArea = allRunners
        .filter(runner => {
