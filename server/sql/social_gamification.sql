...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='likes' AND column_name='post_id') THEN
        ALTER TABLE likes ADD COLUMN post_id INTEGER REFERENCES posts(id) ON DELETE CASCADE;
    END IF;
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'push_tokens_user_id_token_key') THEN
...
-- Add valid_date column if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='challenges' AND column_name='valid_date') THEN
        ALTER TABLE challenges ADD COLUMN valid_date DATE;
    END IF;
END $$;
